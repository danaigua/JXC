 ...
 */
@RestController
@RequestMapping("/admin/ckGoodsType")
public class CkGoodsTypeAdminController {
	...
	public Map<String, Object> save(String name, Integer parentId) throws Exception{
		...
		CkGoodsType ckGoodsType = new CkGoodsType();
		ckGoodsType.setName(name);
		ckGoodsType.setState(0);
		ckGoodsType.setpId(parentId);
		ckGoodsType.setIcon("icon-forder");
		ckGoodsTypeService.save(ckGoodsType);
		CkGoodsType parentCkGoodsType = ckGoodsTypeService.findById(parentId);
		parentCkGoodsType.setState(1);//设置为非也在节点
	...
	public Map<String, Object> delete(Integer id) throws Exception{
		Map<String, Object> map = new HashMap<String, Object>();
		if(ckGoodsService.findByTypeId(id).size() == 0) {
			CkGoodsType currentCkGoodsType = ckGoodsTypeService.findById(id);
			if(ckGoodsTypeService.findByParentId(currentCkGoodsType.getpId()).size() == 1) {
				CkGoodsType parentCkGoodsType = ckGoodsTypeService.findById(currentCkGoodsType.getpId());
				parentCkGoodsType.setState(0);
				ckGoodsTypeService.save(parentCkGoodsType);
			}
			logService.save(new Log(Log.UPDATE_ACTION,"删除商品类别信息" + currentCkGoodsType));
			ckGoodsTypeService.delete(id);
			map.put("success", true);
		}else {
	...
	public JsonArray getByParentId(Integer parentId){
		List<CkGoodsType> ckGoodsTypeList=ckGoodsTypeService.findByParentId(parentId);
		JsonArray jsonArray=new JsonArray();
		for(CkGoodsType ckGoodsType:ckGoodsTypeList){
			JsonObject jsonObject=new JsonObject();
			jsonObject.addProperty("id", ckGoodsType.getId()); // 节点Id
			jsonObject.addProperty("text", ckGoodsType.getName()); // 节点名称
			if(ckGoodsType.getState()==1){
				jsonObject.addProperty("state", "closed"); // 根节点
			}else{
				jsonObject.addProperty("state", "open"); // 叶子节点
			}
			jsonObject.addProperty("iconCls", ckGoodsType.getIcon()); // 节点图标
			JsonObject attributeObject=new JsonObject(); // 扩展属性
			attributeObject.addProperty("state", ckGoodsType.getState()); // 节点状态
